...
if (Test-Path $CODEX_PYTHON) {
    ...
    if ($has_python) {
        Write-Host "[ระบบ] ตรวจพบ Python ในระบบหลัก" -ForegroundColor Green
    } else {
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ระบบ] กำลังติดตั้งไลบรารีที่จำเป็น..." -ForegroundColor Yellow
    $REQ_PATH = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) "requirements.txt"
